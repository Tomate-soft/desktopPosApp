...
import GenericPaymentInterface from '@renderer/components/genericPaymentInterface/genericPaymentInterface'
...
export default function MoreActionsMenu({
  ...
  const authorizations = authData?.payload?.user?.authorizations?.pos?.sellTypes?.restaurant ?? []
                ...
                if (element?.enable?.includes(item.status)) {
                  return (
                    <button
                      key={index}
                      style={
                        selectedOption === element.set ? { background: 'white', color: '#000' } : {}
                      }
                      onClick={() => {
          ...
          {selectedOption === BILL_NAME ? (
            <>
              {authorizations.includes(CHANGE_BILL_NAME_AUTH) ? (
                ...
                <ValidateAuthMessage allow={false}>No autorizado</ValidateAuthMessage>
              )}
            </>
          ) : selectedOption === NOTES_NAME ? (
            <>
              {item.bill[0].notes.length && authorizations.includes(CHANGE_NOTE_NAME_AUTH) ? (
                <ActionsKeyboard
                  option={selectedOption}
                ...
                <ValidateAuthMessage allow={authorizations.includes(CHANGE_NOTE_NAME_AUTH)}>
            ...
            <>
              {authorizations.includes(SEPARATE_NOTES_AUTH) ? (
                <SeparateChecks item={item} openModal={confirmChanges.openModal}></SeparateChecks>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(SEPARATE_NOTES_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(TRANSFER_PRODUCTS_AUTH) ? (
                <TransferProducts item={item} openModal={confirmChanges.openModal}>
                  ""
                </TransferProducts>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(TRANSFER_PRODUCTS_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(MOVE_TABLE_AUTH) ? (
                ...
                <ValidateAuthMessage allow={authorizations.includes(MOVE_TABLE_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(DISCOUNT_PRODUCTS_AUTH) ? (
                <ProductsDiscounts item={item} openModal={confirmChanges.openModal}>
                  YEP
                </ProductsDiscounts>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(DISCOUNT_PRODUCTS_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {item.bill[0].notes.length && authorizations.includes(DISCOUNT_NOTES_AUTH) ? (
                <NotesDiscounts item={item} openModal={confirmChanges.openModal}>
                  YEP
                </NotesDiscounts>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(DISCOUNT_NOTES_AUTH)}>
                  No se encuentran notas activas
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(DISCOUNT_BILL_AUTH) ? (
                ...
                <ValidateAuthMessage allow={authorizations.includes(DISCOUNT_BILL_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(COURTESY_PRODUCTS_AUTH) ? (
                <ProductsCourtesy item={item} openModal={confirmChanges.openModal}>
                  YEP
                </ProductsCourtesy>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(COURTESY_PRODUCTS_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {item.bill[0].notes.length && authorizations.includes(COURTESY_NOTE_AUTH) ? (
                <NotesCourtesy item={item} openModal={confirmChanges.openModal}>
                  YEP
                </NotesCourtesy>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(COURTESY_NOTE_AUTH)}>
                  No se encuentran notas activas
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(COURTESY_BILL_AUTH) ? (
                ...
                <ValidateAuthMessage allow={authorizations.includes(COURTESY_BILL_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(CANCEL_PRODUCT_AUTH) ? (
                ...
                <ValidateAuthMessage allow={authorizations.includes(CANCEL_PRODUCT_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {item.bill[0].notes.length && authorizations.includes(CANCEL_NOTE_AUTH) ? (
                <NotesCancellation item={item} openModal={confirmChanges.openModal}>
                  YEP
                </NotesCancellation>
              ) : (
                <ValidateAuthMessage allow={authorizations.includes(CANCEL_NOTE_AUTH)}>
                  No se encuentran notas activas
                </ValidateAuthMessage>
              )}
            </>
            ...
            <>
              {authorizations.includes(CANCEL_BILL_AUTH) ? (
                ...
                <ValidateAuthMessage allow={authorizations.includes(CANCEL_BILL_AUTH)}>
                  No autorizado
                </ValidateAuthMessage>
              )}
            </>
