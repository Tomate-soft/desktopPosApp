...
import { useUsersStore } from '@renderer/store/users.store'
import { useEffect, useState } from 'react'
import { usePayStore } from '@renderer/store/payments/payments.store'
import { useOperationProcess } from '@renderer/store/operatingPeriod/operatingPeriod.store'
...
export default function CashMoves({ onClose, openModal }: Props) {
  ...
  const tipEnable = {
    tipAmount: selectUser?.tips
      ?.filter((tip) => tip.paymentType === 'debit' || tip.paymentType === 'credit')
        ...
        <div>
          ...
          <div>
            {managementUsers.map((element) => (
              <div
                className={selectUser._id === element._id ? styles.userLine : styles.hidden}
                key={element.id}
