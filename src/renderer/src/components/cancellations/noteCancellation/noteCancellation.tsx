...
export default function NotesCancellation({ item, openModal, children }: Props) {
  const authData = useAuthStore((state) => state.authData)
  const [selectedNote, setSelectedNote] = useState()
  const genericKeyboard = useModal(GENERIC_KEYBOARD_ACTIVE)
  const managementNotes = item.bill[0]?.notes.filter((element) => element.status === ENABLE_STATUS)
          ...
          <div>
            ...
            <div>
              <div className={styles.head}>
                <div>
                  {item.bill[0]?.notes ? <span>{`Mesa ${item.tableNum}`}</span> : null}
