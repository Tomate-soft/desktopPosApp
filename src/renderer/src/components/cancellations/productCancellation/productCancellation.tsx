...
export default function ProductsCancel({ item, openModal, children }: Props) {
  ...
  // zustand
  const cancellationReasonsArray = cancellationReasonStore((state) => state.reasonsArray)
  const cancelProduct = UseActions((state) => state.cancelProduct)
  const getReasons = cancellationReasonStore((state) => state.getReasons)
  const managementProducts = (selectedNote?.products || item.bill?.[0]?.products || []).filter(
    (product) => product.unique !== productSelection?.unique
  )
