...
export default function SeparateChecks({ item, openModal }: Props) {
  ...
  const managementNotes = separateNotes.filter((element) => element.status === ENABLE_STATUS)
            ...
            {separateNotes && separateNotes.length > 0 ? (
              <>
                  ...
                  <div>
                    ...
                    <div>
                      <button /*   enableNote.some(
                        ...
                        disabled={
                          (enableNote.length === 0 && selectedProducts.length === 0) ||
                          selectedProducts.length === 0 ||
                          (noteElement.products.some((prod: any) =>
                            selectedProducts.some((selProd: any) => selProd.unique === prod.unique)
                          ) &&
                            selectedProducts &&
                            selectedProducts.length > 0)
                        }
                          ...
                          if (selectedProducts.length > 0) {
                            setSeparateNotes((prevSeparateNotes) => {
                              ...
                              if (index !== -1) {
                                ...
                                updatedNotes[index].products.push(...selectedProducts)
                                updatedNotes.forEach((note, i) => {
                                  if (i !== index) {
                                    updatedNotes[i].products = updatedNotes[i].products.filter(
                ...
                <div>
                  <button
                    onClick={() => {
                      const updateNotes = [...separateNotes, { ...NOTE_TEMPLATE }]
                  ...
                  <button
                    disabled={
                      separateNotes.length <= 2 || separateNotes[separateNotes.length - 1]._id
                    }
                    onClick={() => {
                      const productsLastNote = [...separateNotes[separateNotes.length - 1].products]
                      const firstNoteProducts = [...separateNotes[0].products]
                      const updatedFirstNoteProducts = firstNoteProducts.concat(productsLastNote)
