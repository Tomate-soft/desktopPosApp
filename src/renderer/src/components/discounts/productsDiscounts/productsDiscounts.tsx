...
export default function ProductsDiscounts({ item, openModal, children }: Props) {
  ...
  const removeNotePorductDiscount = UseActions((state) => state.deleteNoteProductDiscount)
  const removeBillPorductDiscount = UseActions((state) => state.deleteBillProductDiscount)

  const discountApply =
    mode === SET_PERCENT
      ? ((parseFloat(productSelection?.priceInSite) * parseFloat(percent)) / 100).toString()
      : (parseFloat(productSelection?.priceInSite) - parseFloat(percent)).toString()
  ...
  return (
    <div className={styles.container}>
      <div className={styles.discountContainer}>
        <div>
                      ...
                      <div className={styles.productBox} key={index}>
                        ...
                        {element.discount ? (
                          <button
                            ...
                            onClick={() => {
                              const updateProducts = selectedNote.products.filter(
                              ...
                              if (element.discount.setting === 'SET_PERCENT') {
                                if (element.quantity > 1) {
                                  const originalPrice =
                                    ...
                                    (1 - parseFloat(element.discount.discountMount) / 100)
                                  ...
                                  const sendProducts = [...updateProducts, currentProduct]
                          ...
                          <input
                            ...
                            onChange={() => {
                              ...
                              console.log(
                                parseFloat(
                                  selectedNote?.checkTotal || item.bill[0]?.checkTotal || '0'
                      ...
                      <div className={styles.productBox} key={index}>
                        ...
                        {element.discount ? (
                          <button
                            className={styles.discountButton}
                            onClick={() => {
                              const updateProducts = item.bill[0].products.filter(
                                (product) => product.unique != element.unique
                              )
                              if (element.discount.setting === 'SET_PERCENT') {
                                if (element.quantity > 1) {
                                ...
                                const currentProduct = {
                                  ...element,
                                  discount: null,
                                  priceInSite: originalPrice.toFixed(2).toString()
                                }
                                // aca vamos a juntar los productos y mandarlos
                                const sendProducts = [...updateProducts, currentProduct]
                                const checkTotalNew = sendProducts
          ...
          <div>
            ...
            <DiscountBoard mode={mode} settingMode={setMode} percent={percent} setting={setPercent}>
              ea la marea
            </DiscountBoard>
          </div>
        </div>
        <div>
          <button
            ...
            disabled={
              ...
              parseFloat(discountApply) >= parseFloat(productSelection?.priceInSite) - 1 ||
      ...
      {genericKeyboard.isOpen && genericKeyboard.modalName === GENERIC_KEYBOARD_ACTIVE ? (
        <>
          <GenericKeyboard
