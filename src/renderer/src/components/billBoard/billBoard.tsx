...
import { FilterType, OrderType, SELL_TYPE_ORDER, filters, headers } from './lib'
...
export default function BillBoard({ isOpen, onClose, children }: Props) {
  ...
  const filterTables = tableArray.filter((table) => table.status === ENABLE_STATUS)
            ...
            <div>
              {filters.map((element) => (
                <button
                  ...
                  style={
                    filter === element.value ? { background: '#ffffffa0', color: 'black' } : {}
                  }
                >
