  ...
  const getUsers = async () => {
    ...
    try {
      ...
      if (!res.data) {
        setIsLoading(false)
        setErrors(true)
        throw new Error('No se pudo resolver la peticion, usuarios no encontrado')
      }
      ...
      console.error(`No se pudo resolver la peticion debido a un error inesperado ${error}`)
    } finally {
      setIsLoading(false)
    }
  }
