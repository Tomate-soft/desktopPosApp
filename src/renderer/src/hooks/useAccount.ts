...
import { BILLS_PATH, DEVICE_IDN_PATH, TABLES_PATH } from '../lib/routes.paths.lib'
...
export default function UseAccount() {
  ...
  */
  ...
  async function updateBill(statusChange: string, currentBill: any, form?: Bill) {
    /*   tables.map((item) => {
  ...
  const handlePrint = async (process: string, billPrint: Bill | undefined) => {
    ...
    // deberiamos traer la configuracion de un archiuvo local de configuracion
    ...
    try {
      await axios.post(`http://localhost:8114/print/${process}`, data)
