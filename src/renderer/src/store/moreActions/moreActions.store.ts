...
import { cancelBillService, cancelProductService } from '../../services/cancellation.store'
  ...
  return {
    ...
    moveBill: async (data) => {
      ...
      try {
        ...
        if (!secondRequestRes.data) {
          try {
            const res = await SaveBillInTableService(data.idHost, data.receivingTable)
            set({
              isLoading: false,
              errors: true,
            ...
            throw new Error('No se pudo realizar el cambio de mesa, se han revertido los cambios')
          } catch (error) {
        ...
        throw new Error(`Ha ocurrido algo inesperado. Mas informacion: ${error}`)
      }
    },
    ...
    transferProducts: async (data) => {
      ...
      try {
        ...
        throw new Error(`No se actualizo debido a un error inesperado, mas informacion: ${error}`)
      }
    },
