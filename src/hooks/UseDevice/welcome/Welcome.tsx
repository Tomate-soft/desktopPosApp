...
import UseBussines from "@/store/bussines/bussines.store";
  ...
  return (
    <main className={styles.screen}>
      <div>
                  ...
                  onClick={async () => {
                    if (element?.status === true) {
                      const register = await fetch(
                        "http://localhost:3000/create-config",
                        {
                          method: "POST",
                          headers: {
                        ...
                        .then(async (data) => {
                          if (
                            data.message === "Archivo creado correctamente."
                          ) {
        ...
        <footer>
          Necesitas ayuda? Consulta nuestro portal de atención en tomatesoft.com
        </footer>
        {confirmChanges.isOpen && confirmChanges.modalName ? (
          <ConfirmChanges
            ...
            actionType={() => {
              updateDevice(device?._id, { status: false }),
